    ...
    async def run(self) -> None:
        ...
        while not self._stop:
            try:
                ...
                print("[WS] 会话已过期，停止重连。请重新登录后运行 scripts/export_cookies.py")
                if self.on_expired:
                    self.on_expired()
                return
                ...
                if attempt >= self.reconnect.max_attempts:
                    print(f"[WS] 连接失败达到 {self.reconnect.max_attempts} 次，停止重连: {type(e).__name__}")
                    return
                delay = min(
                    self.reconnect.initial_delay * (self.reconnect.backoff_factor ** (attempt - 1)),
                    self.reconnect.max_delay,
                )
                print(f"[WS] 连接断开({type(e).__name__})，{delay:.1f}s 后重试 ({attempt}/{self.reconnect.max_attempts})")
